...
public class TennisCourtReservations{
    ...
    /** First argument is the number of days to add to the current date */
    static void main(String[] args) throws IOException, InterruptedException{
        ...
        for(String court : COURTS.keySet()){
            String resStr = makeHttpRequest(client, COURTS.get(court) + date);
            IO.println(court);
    ...
    static String makeHttpRequest(HttpClient httpClient, String url) throws IOException, InterruptedException{
        for(int i = 0; i < 2; i++){//request sometimes fails on the first try, so retrying before failing
